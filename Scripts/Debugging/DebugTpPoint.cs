...
public class DebugTpPoint : MonoBehaviour
{
    ...
    [SerializeField] UnityEvent _updateTpPoint;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            _updateTpPoint.Invoke();
        }
    }

    public void Tp(GameObject player, GameObject boulder)
    {
        player.transform.position = _playerTpPoint;
        boulder.transform.position = _boulderTpPoint;

        //reset player and boulder states
        PlayerState playerState = player.GetComponent<PlayerState>();
        BoulderController boulderController = boulder.GetComponent<BoulderController>();

        playerState.ChangeStateToRun();
        boulderController.ChangeStateToFreefall();
    }
